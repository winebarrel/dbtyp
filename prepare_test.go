...
func TestTxPrepare(t *testing.T) {
	...
	stmt, err := tx.PrepareContextT(t.Context(), "insert into foo values (?)")
	var _ iface.Stmt = stmt
	require.NoError(err)
	_, err = stmt.ExecContext(t.Context(), 100)
	require.NoError(err)
	...
	var n int
	err = db.QueryRowContext(t.Context(), "select id from foo").Scan(&n)
	require.NoError(err)
	assert.Equal(100, n)
}
